...
List_Elem* add_front(List *s, char* val)
{
...
List_Elem* add_back(List* s, char* val)
{
    //CARCKIYassert(list_ok(s));
...
List_Elem* add_after(List* s, char* val, List_Elem* cur_el)
{
...
List_Elem* add_before(List* s, char* val, List_Elem* cur_el)
{
...
char* list_remove (List* s, List_Elem* cur_el)
{
    CARCKIYassert(list_ok(s));
...
List_Elem* find_val(List* s, char* val)
{
    List_Elem* cur_el = s->head;
    while (strcmp(cur_el->value, val))
    {
        if (cur_el->next == NULL)
        {
    ...
    /* while (cur_el->value != NULL && val != NULL && strcmp(cur_el->value, val) == 0)
    {
        if (cur_el->next != NULL && cur_el->value != NULL && val != NULL)
            cur_el = cur_el->next;
        else
            break;
    }
